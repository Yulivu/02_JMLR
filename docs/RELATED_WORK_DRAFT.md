...
Constrained decoding and WFST-style methods are a natural point of comparison because they also combine sequence models with symbolic structure. In these methods, the central object is usually the decoded output under a constraint: the inference procedure searches over legal paths, repairs invalid outputs, or restricts the output space to satisfy a formal rule. This is the right tool when the goal is output validity.
...
Confidence calibration studies whether model probabilities correspond to empirical correctness. This is related because both calibration and posterior event mass involve probabilities attached to model behavior. However, calibration typically concerns the reliability of predicted confidence values, while our question is whether a structured rule receives posterior mass.
...
Lagrangian relaxation and constrained optimization methods introduce penalties or dual variables to solve constrained objectives. These methods are optimization tools. They are useful when the goal is to enforce or approximately enforce constraints during inference or learning.
