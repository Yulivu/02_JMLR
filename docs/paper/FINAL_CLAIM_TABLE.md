...
| C8 | Low `P_theta(L|x)` is a field-style rule-specific risk signal | Main-with-boundary | R6a bottom/top gaps plus reanalysis: AUROC `0.7088`, AUPRC `0.8470`; uncertainty baseline rerun shows event risk is positive but not strongest | field-style tasks only; not calibration, not all structured prediction, not uncertainty-baseline superiority |
