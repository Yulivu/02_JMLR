...
| --- | --- | --- | --- | --- |
| C1 | `P_theta(L|x)` is a well-defined CRF posterior event probability for finite `Y,T` and regular language `L` | Main | `docs/evidence/THEORY_AUDIT.md`; posterior algebra tests | requires finite sequence length and finite label set |
